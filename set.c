...
#define NEW_SET (set *)malloc(sizeof(set))
#define NEW_CHILD (child *)malloc(sizeof(child))
...
void setadd(child *set1, child *set2, char *str) {
    ...
    if(set1==set2){
        while(set1!=NULL){
            strcat(str, " ");
            strcat(str, get_name(set1->child_id));
            set1=set1->next;
        }
        return;
    }
